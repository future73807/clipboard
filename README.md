...
### 界面与布局
- [x] 主面板宽度是否 100% 占满窗口？ (`w-full h-screen`)
...
- [x] **图片**: 缩略图是否正常显示？OCR 是否正常工作？ (tesseract.js)
- [x] **代码**: 语言检测是否准确？语法高亮是否正确？ (react-syntax-highlighter)
- [x] **文件**: 文件图标是否正确显示？路径复制是否正常？ (FileIcon 组件)
- [x] **快捷方式**: 应用启动是否正常？ (shortcut.ts)
- [x] **密码**: 加密存储是否正确？ (AES-256-GCM)
- [x] **办公文件**: 预览是否正常？ (mammoth, xlsx, pdfjs)
...
### 系统集成
- [x] 右键菜单是否正确注册？ (contextMenu.ts)
- [x] 复制监听确认条是否正常弹出？ (CopyConfirmBar.tsx)
- [x] 全局快捷键是否正常响应？ (Ctrl+Shift+V, Ctrl+Alt+C)

### 安全性
- [x] 密码是否默认 AES-256 加密？ (crypto.ts)
- [x] 加密密钥是否安全存储？ (PBKDF2 派生)
- [ ] 敏感数据是否在内存中及时清除？ (待优化)
